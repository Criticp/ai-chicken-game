...
# Territory & Tactics Upgrade
from .territory_engine import TerritoryEvaluator, TacticalSuicideEvaluator
...
class NeuralEvaluator:
    ...
    def __init__(self, trapdoor_tracker):
        ...
        # Territory & Tactics Upgrade
        self.territory_evaluator = TerritoryEvaluator()
        self.tactical_suicide = TacticalSuicideEvaluator()

        # Fallback weights (if NN not loaded)
        self.fallback_weights = {
            'egg_diff': 7.6488,
            'mobility': 0.1693,  # OLD: Simple move counting
            'territory_control': 1.0,  # NEW: Flood fill area control
            'corner_proximity': 2.5,
            'turd_diff': 0.3354,
            'trapdoor_risk': -20.0,  # Will be dynamic now
            'intercept': -0.0055,
        }
    ...
    def _heuristic_evaluate(self, board: "game_board.Board") -> float:
        ...
        # NEW: Territory control via flood fill
        # Directive: Replace "Move Count" with "Reachable Area"
        my_area, enemy_area, territory_score = self.territory_evaluator.evaluate_control(board)

        # Keep simple mobility as tiebreaker
        my_moves = len(board.get_valid_moves(enemy=False))
        enemy_moves = len(board.get_valid_moves(enemy=True))
        mobility_diff = my_moves - enemy_moves
        ...
        # OLD: Static trap risk
        # my_risk = self.tracker.get_trapdoor_risk(my_loc)

        # NEW: Dynamic trap penalty (Tactical Suicide)
        # If far from spawn with fertile territory, trap becomes viable
        my_risk = self.tracker.get_trapdoor_risk(my_loc)
        if my_risk > 0.1:  # Only calculate if there's actual trap risk
            # Get dynamic penalty (can be positive if teleporting is profitable!)
            trap_squares = []
            for y in range(8):
                for x in range(8):
                    if self.tracker.get_trapdoor_risk((x, y)) > 0.5:
                        trap_squares.append((x, y))

            if trap_squares:
                # Use closest likely trap
                closest_trap = min(trap_squares,
                                 key=lambda t: abs(t[0]-my_loc[0]) + abs(t[1]-my_loc[1]))
                dynamic_penalty = self.tactical_suicide.calculate_dynamic_trap_penalty(
                    board, my_loc, closest_trap
                )
            else:
                dynamic_penalty = self.fallback_weights['trapdoor_risk'] * my_risk
        else:
            dynamic_penalty = 0.0
        ...
        # Linear combination with NEW territory score
        score = (
            self.fallback_weights['egg_diff'] * egg_diff +
            self.fallback_weights['mobility'] * mobility_diff * 0.5 +  # Reduced weight
            self.fallback_weights['territory_control'] * territory_score +  # NEW: Major factor
            self.fallback_weights['corner_proximity'] * corner_diff +
            self.fallback_weights['turd_diff'] * turd_diff +
            dynamic_penalty +  # NEW: Dynamic instead of static
